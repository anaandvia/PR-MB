...
use PHPMailer\PHPMailer\PHPMailer;
    ...
    // update
    if ($status_kembali == 1){
        $sql = mysqli_query($koneksi, "UPDATE pengembalian SET status_kembali='$status_kembali', alasan = '$alasan' , tgl_acc_back = '$tgl' WHERE id='$id'");
        ...
        // kondisi
            if($sql && $sql2 && $sql3 == true){
                ...
                $mail->Subject = 'Pengembalian Ruangan DI KONFIRMASI | PR-MB';

                //Read an HTML message body from an external file, convert referenced images to embedded,
                //convert HTML into a basic plain-text alternative body
                $body = "Hi, ".$nama."<br>Pengembalian Ruangan Telah <strong style='color:green;'>DI KONFIRMASI </strong>";
        ...
        $sql = mysqli_query($koneksi, "UPDATE pengembalian SET status_kembali='$status_kembali', alasan = '$alasan',tgl_acc_back = '$tgl' WHERE id='$id'");
        $sql2 = mysqli_query($koneksi,"UPDATE peminjaman SET status='1' WHERE id_peminjaman='$id_peminjaman'");
        $sql3 = mysqli_query($koneksi,"UPDATE ruangan SET status='0' WHERE id_ruangan ='$id_ruangan'");
        if ($sql && $sql2 && $sql3 == true){
        //Create a new PHPMailer instance
        $mail = new PHPMailer;
        ...
        //Set the subject line
        $mail->Subject = 'Pengembalian Ruangan DI TOLAK | PR-MB';

        //Read an HTML message body from an external file, convert referenced images to embedded,
        //convert HTML into a basic plain-text alternative body
        $body = "Hi, ".$nama."<br>Pengembalian Ruangan <strong style='color:red;'>DI TOLAK </strong>
        <br> Alasan : ".$alasan." 
        <br> Silahkan Ulangi Pengembalian Ruangan di :

        <br> http://localhost/PR-MB/user/view/history.php" ;
        $mail->Body = $body;
        //Replace the plain text body with one created manually
        $mail->AltBody = 'Pengembalian Ruangan';
